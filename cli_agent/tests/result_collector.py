...
from test_config import REPORTS_DIR, CSV_OUTPUT_FILE, REPORT_OUTPUT_FILE, JSON_OUTPUT_FILE
...
class ResultCollector:
    ...
    def __init__(self, worker_id: Optional[str] = None):
        ...
        self.worker_file = self.reports_dir / f"worker_{self.worker_id}_{os.getpid()}.json"
    ...
    def _load_all_worker_results(self) -> List[TestResult]:
        ...
        worker_files = list(self.reports_dir.glob("worker_*.json"))
    ...
    def generate_csv_report(self, filename: Optional[str] = None) -> str:
        ...
        with open(filepath, 'w', newline='', encoding='utf-8') as csvfile:
            if not self.results:
                ...
                writer.writerow([
                    'test_id', 'test_type', 'model', 'input_prompt', 'agent_output', 
                    'passed', 'failure_reason', 'grader_results', 'execution_time', 'timestamp'
                ])
                return str(filepath)
            
            fieldnames = [
                'test_id', 'test_type', 'model', 'input_prompt', 'agent_output', 
                'passed', 'failure_reason', 'execution_time', 'timestamp'
            ]
            
            # Add grader result columns dynamically
            all_grader_keys = set()
            for result in self.results:
                if result.grader_results:
                    all_grader_keys.update(result.grader_results.keys())
            
            grader_columns = sorted(all_grader_keys)
            fieldnames.extend(grader_columns)
            ...
            for result in self.results:
                row = asdict(result)
                # Remove grader_results dict and flatten into individual columns
                grader_results = row.pop('grader_results', {}) or {}
                
                # Add grader results as individual columns
                for grader_key in grader_columns:
                    row[grader_key] = grader_results.get(grader_key, '')
    ...
    def generate_summary_report(self, filename: Optional[str] = None) -> str:
        ...
        report_lines = [
            ...
            f"- **Overall Pass Rate:** {sum(1 for r in self.results if r.passed) / len(self.results) * 100:.1f}%" if self.results else "- **Overall Pass Rate:** N/A",
            "",
            "## Pass Rate by Model",
            "",
        ]
        ...
        report_lines.extend([
            ...
            f"- **Average Execution Time:** {sum(r.execution_time or 0 for r in self.results) / len(self.results):.2f}s" if self.results else "- **Average Execution Time:** N/A",
        ])
...
def generate_all_reports():
    ...
    report_path = temp_collector.generate_summary_report(f"test_summary_report_{timestamp}.md")
