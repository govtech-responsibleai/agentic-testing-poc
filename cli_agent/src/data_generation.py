        ...
        products.append(
            {
                ...
                + " "
                + random.choice(["Pro", "Plus", "Basic", "Deluxe"]),
...
def build_pdfs(fake: Faker):
    ...
    for i in range(15):
        ...
        c.drawString(
            100,
            685,
            f"Due Date: {(datetime.strptime(invoice_date, '%Y-%m-%d') + timedelta(days=30)).strftime('%Y-%m-%d')}",
        )
        ...
        for j in range(random.randint(1, 5)):
            item_name = (
                fake.word().title()
                + " "
                + random.choice(["Supplies", "Equipment", "Materials"])
            )
            qty = random.randint(1, 10)
